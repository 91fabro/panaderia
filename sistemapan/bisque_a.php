        ...
        <nav class="navbar navbar-inverse navbar-static-top">
          ...
          <ul class="dropdown-menu">
           <li ><a href="migajon_a.php">Gramos de Migajon </a></li>
            <li><a href="bambazo_a.php">Gramos para Pambazo</a></li>
            <li><a href="Bolillo_a.php">Gramos para Bolillo</a></li>
...
<!-- <button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-pencil"></span> Star  </button> -->
  ...
  <div class="alert alert-info"> <center> Lista de Gramos Registrados para Masa Polvoron</center> </div>
...
<!--  masa migajon   -->
 <button type="button" class="btn btn-info btn" data-toggle="modal" data-target="#masa_migajon">Masa de polvoron</button></br>
 ...
 <div class="modal fade" id="masa_migajon" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Masa de Leche Nescle</h4>
             <p> Rellena la informacion Solicitada</p>
        </div>
        <div class="modal-body">
    <center>
...
manteca
colorante
esencia mantequilla o vainilla
...
echo $tipo=$_REQUEST['gramosp']; echo  $nomi=$_REQUEST['idpang'];
 $xsqla=mysqli_query($conexion,"SELECT * FROM masa_bisque WHERE cantidad='1'");
  while($row = mysqli_fetch_array($xsqla))
             {
                $uharina= $row['harina']; $uazucar= $row['azucar'];$uamanteca= $row['manteca']; $royal= $row['royal']; $mlla= $row['mantequilla'];
                $e_mant= $row['e_mant'];$levadura= $row['levadura'];$agua= $row['agua'];$huevo= $row['huevo'];
             }
 $gharina=$tipo* $uharina;   $gazucar=$tipo* $uazucar;     $gmanteca=$tipo* $uamanteca; $groyal=$tipo* $royal;    
     $gmlla=$tipo* $mlla; $gesencia=$tipo* $e_mant;    $glevadura=$tipo* $levadura;      $gagua=$tipo* $agua; $ghuevo=$tipo* $huevo;          
       


 $result = mysqli_query($conexion,"INSERT INTO  masa_bisque(nombre_insumo,cantidad,harina,azucar,manteca,royal,mantequilla,e_mant,levadura,agua,huevo)
                                    VALUES ('$nomi','$tipo','$gharina','$gazucar','$gmanteca','$groyal','$gmlla','$gesencia','$glevadura','$agua','$huevo');");
...
*/
  ...
  <table  class="table table-hover"    id="tabla_lista_paises" >
    <thead >
      ...
      <th>Nombre </th>    <th>cantidad </th><th>Harina </th> <th>Azucar </th> <th>Manteca</th> <th>Royal</th>  <th>Colorante</th>
       <th>ecencia Mantequilla</th>         <th>ecencia de Vainilla</th>
    ...
    <tbody>
      ...
      <?PHP include("conexion.php");
             ...
             $resultx = mysqli_query($conexion,"SELECT * FROM masa_polvoron ORDER BY  nombre_insumo ASC;");
            while($row = mysqli_fetch_array($resultx))
             {
       ?>      <tr>
                   <td ALIGN='CENTER' width='100'><?php echo($row["nombre_insumo"]); ?> </td> <td ALIGN='CENTER' width='100'><?php echo($row["cantidad"]); ?> 
                   <td ALIGN='CENTER' width='100'><?php echo($row["harina"]); ?> Gr </td><td ALIGN='CENTER' width='100'><?php echo($row["azucar"]); ?> Gr </td>
                 <td ALIGN='CENTER' width='100'><?php echo($row["manteca"]); ?> Gr </td><td ALIGN='CENTER' width='100'><?php echo($row["royal"]); ?> Gr </td>
                 <td ALIGN='CENTER' width='100'><?php echo($row["colorante"]); ?> Gr  </td><td ALIGN='CENTER' width='100'><?php echo($row["e_mant"]); ?> Gr </td>
                 <td ALIGN='CENTER' width='100'><?php echo($row["e_vainilla"]); ?> Gr</td>
 ...
 <?php      
         /**
      $uharina= $row['harina']; $uazucar= $row['azucar'];$uamanteca= $row['manteca']; $royal= $row['royal']; $colorante= $row['colorante'];
                $e_mant= $row['e_mant'];$e_vainilla= $row['e_vainilla'];
         */
